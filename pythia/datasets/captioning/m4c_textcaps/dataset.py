...
class M4CTextCapsDataset(M4CTextVQADataset):
    ...
    def preprocess_sample_info(self, sample_info):
        ...
        if 'caption_id' in sample_info:
            sample_info['question_id'] = sample_info['caption_id']    ## added for qa dataset joint train
        return sample_info
    ...
    def add_answer_info(self, sample_info, sample):
        ...
        elif self._dataset_type!='test':    ## added for qa dataset joint train
            sample_info['answers'] = [sample_info['answers'][0]]
        ...
        elif self._dataset_type!='test':    ## added for qa dataset joint train; only in pretrain, not used anyway, just match the format
            sample.caption_str = enc_obj2bytes(sample_info['answers'])
            sample.ref_strs = enc_obj2bytes([sample_info['answers']])
            sample.pop('gt_answers_enc')
